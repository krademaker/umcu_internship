...
# ABOUT: Script to process EA (Educational Attainment) summary statistics data with (1) MAF > 0.01, (2) P < 0.05, (3) bi-allelic variants, (4) presence in 1000 Genomes Project (1KGP)
...
log_file=${out_dir}/log_ea.txt
output_file=${out_dir}/processed_ea_2018_sum_stats.txt
depict_file=${out_dir}/depict_ea_2018.txt

cp $HOME/Koen/GWAS_data/sum_stats/ea_2018_sum_stats.txt ${work_dir} # Copy summary statistics to work directory
cp $HOME/1K/g1000_eur.synonyms ${work_dir} # Copy SNP synonyms to work directory
cd ${work_dir} # Move to work directory
head -n 1 ea_2018_sum_stats.txt > tmp_header.txt


# (2) Data processing
	# (2a) MAF filter
awk '$6>0.01 && $6<0.99' ea_2018_sum_stats.txt > tmp_maf.txt # Filter out SNPs with MAF < 0.01
	# (2b) P filter
...
awk 'NR == FNR{c[$1]++;next};c[$1] > 0' g1000_eur.synonyms tmp_maf_p_biallelic.txt > ${output_file} # Filter out SNPs not included in 1KGP list
	# (2e) DEPICT formatting
awk 'BEGIN { OFS = "\t" } FNR>1 {print $1,$2,$3,$9}' ${output_file} > tmp_depict.txt # Extract columns 'SNP', 'CHR', 'BP' and 'P'
...
wc -l ea_2018_sum_stats.txt >> ${log_file} # Original file size
