...
# (1) File organization
work_dir="$TMPDIR"/mdd # Create work directory
mkdir ${work_dir}
out_dir="$TMPDIR"/mdd/output # Create output directory
mkdir ${out_dir}

log_file=${out_dir}/log_mdd.txt
output_file=${out_dir}/processed_mdd_2018_sum_stats.txt
depict_file=${out_dir}/depict_mdd_2018.txt

cp $HOME/Koen/GWAS_data/sum_stats/mdd_2018_sum_stats.txt ${work_dir} # Copy summary statistics to work directory
cp $HOME/1K/g1000_eur.synonyms ${work_dir} # Copy SNP synonyms to work directory
cd ${work_dir} # Move to work directory
head -n 1 mdd_2018_sum_stats.txt > tmp_header.txt


# (2) Data processing
  # (2a) MAF filter
  awk '($6>0.01 && $6<0.99) || ($7>0.01 && $7<0.99)' mdd_2018_sum_stats.txt > tmp_maf.txt # Filter out SNPs with MAF < 0.01
	# (2b) INFO filter
awk '$8>0.9' tmp_maf.txt > tmp_maf_info.txt # Filter out SNPs with INFO <= 0.9
  # (2c) P filter
...
wc -l mdd_2018_sum_stats.txt >> ${log_file} # Original file size
