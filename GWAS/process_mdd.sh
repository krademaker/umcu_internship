...
# TO-DO: Write output file > "$TMPDIR"/mdd/output_mdd/processed_mdd_2018_sum_stats.txt
	# (2f) DEPICT formatting
awk 'BEGIN { OFS = "\t" } FNR>1 {print $2,$1,$3,$11}' "$TMPDIR"/mdd/output_mdd/processed_mdd_2018_sum_stats.txt > mdd_tmp_depict.txt # Extract columns 'SNP', 'CHR', 'BP' and 'P'
awk 'BEGIN { print "SNP\tChr\tPos\tP" }{ print }' mdd_tmp_depict.txt > "$TMPDIR"/mdd/output_mdd/depict_mdd_2018.txt # Rename columns to 'SNP', 'Chr', 'Pos', 'P' for final DEPICT file
...
wc -l "$TMPDIR"/mdd/output_mdd/processed_mdd_2018_sum_stats.txt >> ${summary_file} # Processed file size
wc -l "$TMPDIR"/mdd/output_mdd/depict_mdd_2018.txt >> ${summary_file} # DEPICT file size
